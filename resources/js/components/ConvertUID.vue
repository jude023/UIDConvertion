...
export default {
  ...
  methods:{
      ...
      try {
        ...
        if (phoneNumber) {
          // return response.data;
          // this.saveToNotepad(phoneNumber);
          return phoneNumber;
        }
      } catch (error) {}
    },
      ...
      async MultiConvert() {
          ...
          for(let i = 0; i < totalItems; i++) {
              ...
              if (this.value === 100 || i === totalItems - 1) {
                  ...
                  this.saveToNotepad(this.arrayText)
      ...
      saveToNotepad(data) {
        ...
        const content = data.map(item => 
          `PhoneNumber: ${item.number}, IsValid: ${item.isvalid}`
        ).join('\n');
        console.log(content)
        ...
        const existingContent = localStorage.getItem("phoneNumbersFile");
        ...
        if (existingContent) {
          ...
          updatedContent = [...existingLines, ...data.map(item => 
            `PhoneNumber: ${item.number}, IsValid: ${item.isvalid}`
          )].join('\n');
        } else {
          // First save - create initial content
          updatedContent = content;
        }
        
        // Update localStorage
        localStorage.setItem("phoneNumbersFile", updatedContent);
